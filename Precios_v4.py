# ws_ingestor_last_prices_usd.py (silent)
# -*- coding: utf-8 -*-
...
# ===== Config imprimibles =====
PRINT_UPDATES = (os.getenv("PRINT_UPDATES", "0").strip() == "1")  # por defecto NO imprime updates
VERBOSE       = (os.getenv("VERBOSE", "0").strip() == "1")        # por defecto sin logs
...
def init_connection_eco():
    ...
    vlog(f"Conectado a ECO {ECO_BASE} (LIVE)")

# ===== Tickers =====
def get_all_tickers_from_all_tickers():
    rows = sb.table("all_tickers").select("*").execute().data or []
    uniques, seg_pref = set(), {}
    seg_keys = {"segment","preferredsegment","seg","segmento"}
    for r in rows:
        ...
        chosen_seg = None
        for k, v in r.items():
            if _norm_key(k) in seg_keys:
                chosen_seg = (v or "").strip(); break
        seg_pref[sym] = chosen_seg or DEFAULT_SEG
        uniques.add(sym)
    extras = [_clean_symbol(x) for x in os.getenv("EXTRA_TICKERS","").split(",") if x.strip()]
    for x in extras:
        uniques.add(x); seg_pref.setdefault(x, DEFAULT_SEG)
    uniques.update({"AL30","AL30D"})
    seg_pref.setdefault("AL30", DEFAULT_SEG); seg_pref.setdefault("AL30D", DEFAULT_SEG)
...
def error_handler(msg):
    # solo errores
    print(f"[{_now_local_str()}] WS Error:", msg)

def exception_handler(e):
    print(f"[{_now_local_str()}] WS Exception:", getattr(e, "msg", str(e)))

# ===== Push loop =====
def compute_fx_and_usd(price_ars):
    al30, al30d = _ref_prices.get("AL30"), _ref_prices.get("AL30D")
...
def pusher_loop():
    while not _stop_event.is_set():
        now = time.time()
        with _lock:
            for _, data in list(_latest_by_symbol.items()):
                tk   = data["ticker"]
                ars  = data["last_ars"]
                bid, ask = data.get("bid"), data.get("ask")
                clp  = data.get("closing_price")
                ...
                if (now - last_push) >= PUSH_INTERVAL_SEC:
                    ...
                    upsert_last_prices_row(
                        ticker=tk, price_ars=ars, bid=bid, ask=ask,
                        fx_mep=fx_mep, price_usd=usd,
                        closing_price=clp, change=chg
                    )
                    ...
                    if PRINT_UPDATES:
                        ...
                        cl_txt  = f"{clp:.6f}" if isinstance(clp,(int,float)) else "N/A"
                        print(f"[{_now_local_str()}] UPDATED {tk:8s} ARS={ars:,.6f}  CL={cl_txt}  CHG={chg_txt}  FX={fx_txt}  USD={usd_txt}")
